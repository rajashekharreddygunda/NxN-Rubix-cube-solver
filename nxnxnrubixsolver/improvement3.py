...
def center_piece_arranger(target, position, color, func):  # here color indicates the color of the centrepiece
    ...
    a = position[0]  # a indicates row value of the position of the required piece
    b = position[1]  # b indicates the column value of the position of the required piece
    c = position[2]  # c indicates the face value of the position of the required piece
...
state, N = splitter(
    "ywwwoywyybowbbyooyobowgwg"
    "ybogoogooygorbbyryrywbrbw"
    "gobwbrgwrwrgwryrbrrrrrrrr"
    "yorogogoggwbywbrbwooogyyg"
    "bygybgwgbobrgbobygygbywwr"
    "ygobwbwrbgwyogygwoywrrgrw")
mid_pieces = []

# an empty list to store the coordinates of current piece in the destination position.
# for example [a,b,c] gets stored in mid_pieces[k][i][j]

for k in range(6):  # fills all places with zeros
    mid_pieces.append([])
    for i in range(N):
        mid_pieces[-1].append([])
        for j in range(N):
            mid_pieces[-1][-1].append([0, 0, 0])
for c in range(6):
    dictionary = {"b": 0, "r": 1, "w": 2, "y": 3, "g": 4, "o": 5}
    for a in range(1, N - 1):
        for b in range(1, N - 1):
            if N % 2 != 0 and a / 2 == b / 2 and 2 * a == N - 1:
                pass
            else:
                if mid_pieces[dictionary[state[c][a][b]]][a][b] == [0, 0, 0]:
                    mid_pieces[dictionary[state[c][a][b]]][a][b] = [a, b, c]

                elif mid_pieces[dictionary[state[c][a][b]]][b][N - 1 - a] == [0, 0, 0]:
                    mid_pieces[dictionary[state[c][a][b]]][b][N - 1 - a] = [a, b, c]

                elif mid_pieces[dictionary[state[c][a][b]]][N - 1 - a][N - 1 - b] == [0, 0, 0]:
                    mid_pieces[dictionary[state[c][a][b]]][N - 1 - a][N - 1 - b] = [a, b, c]

                elif mid_pieces[dictionary[state[c][a][b]]][N - 1 - b][a] == [0, 0, 0]:
                    mid_pieces[dictionary[state[c][a][b]]][N - 1 - b][a] = [a, b, c]
for i in mid_pieces:
    print(i)
